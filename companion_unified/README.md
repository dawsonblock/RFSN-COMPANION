...
Notes:
- `credentials.json` should include `api_key` and `agent_name`.
- Moltbook actions are queued for manual approval in the dashboard.

Notes:
- Sending email / creating events require Google OAuth secrets/tokens (see `secrets/` env vars).
- The kernel never sends/creates directly: it only drafts + enqueues.
